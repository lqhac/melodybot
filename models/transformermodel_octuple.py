...
class MusicTransformer(nn.Module):
    ...
    def infer (self, enc_inputs, dec_inputs_gt, sentence_maxlen, temperature, topk, device, output_dir, midi_name, tf_steps=1):
        ...
        for step in tqdm(range(max_sentence_step, decode_length)):
            ...
            bar_logits = bar_out.cpu().squeeze().detach().numpy()
            pos_logits = pos_out.cpu().squeeze().detach().numpy()
            token_logits = token_out.cpu().squeeze().detach().numpy()
            dur_logits = dur_out.cpu().squeeze().detach().numpy()
            phrase_logits = phrase_out.cpu().squeeze().detach().numpy()
            ...
            """
            if token_id == self.event2word_dict['Pitch'][f"</s>"] or \
            bar_id == self.event2word_dict['Bar'][f"</s>"] or \
            pos_id == self.event2word_dict['Pos'][f"</s>"] or \
            dur_id == self.event2word_dict['Dur'][f"</s>"] or \
            phrase_id == self.event2word_dict['Phrase'][f"</s>"]:
                break
            """
            
            """
            if token_id == self.event2word_dict['Pitch'][f"</s>"] and \
            bar_id == self.event2word_dict['Bar'][f"</s>"] and \
            pos_id == self.event2word_dict['Pos'][f"</s>"] and \
            dur_id == self.event2word_dict['Dur'][f"</s>"] and \
            phrase_id == self.event2word_dict['Phrase'][f"</s>"]:
                break
            """
            ...
            dec_inputs['bar'][0, step + 1] = bar_id
            dec_inputs['pos'][0, step + 1] = pos_id
            dec_inputs['token'][0, step + 1] = token_id
            dec_inputs['dur'][0, step + 1] = dur_id
            dec_inputs['phrase'][0, step + 1] = phrase_id
