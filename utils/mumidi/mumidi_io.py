...
def midi2items(file_path_or_midi, instru2track, pitch_shift=0, before_infer_chords=False):
    ...
    items.sort(key=lambda x: (x.start, x.track, x.pitch, -x.end))  # 保留长的note
    items_ = []
    for item in items:
...
def midi_to_training_events_inference(input_path, token2id, instru2track, cond_tracks, tgt_tracks=None):
    try:
        if isinstance(input_path, str):
            item_name = os.path.basename(input_path).split(".")[0]
        else:
