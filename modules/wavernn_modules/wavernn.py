...
class WaveRNN(nn.Module):
    def __init__(self, rnn_dims, fc_dims, bits, pad, upsample_factors,
                 ...
                 hop_length, sample_rate, mode='RAW', use_pitch_embed=False):
        super().__init__()
        self.mode = mode
        self.pad = pad
        if self.mode == 'RAW':
        ...
        self.pitch_embed = Embedding(300, feat_dims, 0)
        self.c_proj = nn.Linear(2 * feat_dims, feat_dims)

    def forward(self, x, mels, p=None):
        ...
        if self.use_pitch_embed:
            mels = self.c_proj(torch.cat([
                mels.transpose(1, 2), self.pitch_embed(p)], -1)).transpose(1, 2)
    ...
    def generate(self, mels, p=None, batched=True, target=11_000, overlap=550):
        device = next(self.parameters()).device  # use same device as parameters
        output = []
        ...
        if batched:
            output = self.xfade_and_unfold(output, target, overlap)[None, ...]
            output = output[:, :out_seq_len]
        return output
